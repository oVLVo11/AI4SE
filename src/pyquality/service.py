...
from .security import CredentialError, CredentialStatus
...
class HarnessService:
        ...
        with self._lock:
            ...
            try:
                future = self._executor.submit(
                    self._loop.resume if resume else self._loop.run, task_id
                )
            except Exception:
    ...
    def approve(self, approval_id: str) -> None:
        self._loop.decide_approval(approval_id, ApprovalDecision.APPROVE)

    def reject(self, approval_id: str) -> None:
        self._loop.decide_approval(approval_id, ApprovalDecision.REJECT)
    ...
    @staticmethod
    def _decode_audit(line: str) -> AuditEvent:
        ...
        if payload["outcome"] is not None:
            safe_metadata["outcome"] = payload["outcome"]
        if payload["duration"] is not None:
            safe_metadata["duration"] = payload["duration"]
        return AuditEvent(
